...
// Matches an opening Twig tag at end of line: [1] opener, [2] whitespace + partial keyword.
const TWIG_TAG_OPENER = /(\{%-?)(\s*\w*)$/;

// Matches a bare word shorthand (2+ chars) not already inside a Twig tag.
const WORD_SHORTHAND_PATTERN = /(?<![%{-])\b(in\w*|em\w*|ex\w*|if\w*|fo\w*|bl\w*|se\w*|ma\w*|ap\w*|wi\w*|ve\w*|im\w*|fr\w*|tr\w*|ca\w*|fi\w*|us\w*)$/i;

/** A Twig snippet completion definition. */
interface SnippetDef {
  /** Label shown in the completion menu. */
  label: string;
  /** Keywords that trigger this snippet in a `{%` context. */
  keywords: string[];
  /** Full snippet text inserted on selection. */
  fullSnippet: string;
  /** Short detail shown beside the label. */
  detail: string;
  /** Sort key controlling ordering in the menu. */
  sortText: string;
  /** Whether this snippet is a generic (non-SDC) Twig snippet. */
  generic: boolean;
}
...
/** Maps a typed shorthand to the snippet label it triggers. */
interface WordTrigger {
  /** The shorthand a user types. */
  shorthand: string;
  /** The `SnippetDef` label this shorthand resolves to. */
  label: string;
}
...
/**
 * Returns tag snippet completions when the cursor is inside an opening `{%` tag.
 *
 * @param lineUpToCursor - Line text from column 0 to the cursor
 * @param lineAfterCursor - Line text from the cursor to end of line
 * @param lineNumber - Zero-based line number
 * @param enableGenericSnippets - Whether to include generic Twig snippets
 * @returns Matching tag snippet completions
 */
export function getTwigTagSnippets(
  ...
  // Consume a trailing auto-closed `}` to avoid producing `%}}`.
  const hasAutoCloseBrace =
    lineAfterCursor.startsWith('}') && !lineAfterCursor.startsWith('%}');
  // Consume an editor auto-inserted `%}` to avoid a duplicate tag close.
...
/**
 * Returns word-level snippet completions for bare Twig shorthands.
 *
 * Must run before any await in the completion handler so the staleness
 * check does not drop results.
 *
 * @param lineUpToCursor - Line text from column 0 to the cursor
 * @param lineNumber - Zero-based line number
 * @param enableGenericSnippets - Whether to include generic Twig snippets
 * @returns Matching word snippet completions
 */
